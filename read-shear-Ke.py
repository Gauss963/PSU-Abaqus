...
for RUPTURE_POSITION in RUPTURE_POSITIONS:
    ...
    np.savetxt(f'./Average/Shear-Stress-S12-{RUPTURE_POSITION}-Y-Averaged.csv', np.column_stack((bin_centers, s12_means)), delimiter=",", header="x,S12", comments="", fmt="%.6e")
    # plt.show()
    plt.close()
    ...
    np.savetxt(f'./Average/Shear-Stress-S11-{RUPTURE_POSITION}-Y-Averaged.csv', np.column_stack((bin_centers, s11_means)), delimiter=",", header="x,S11", comments="", fmt="%.6e")
    ...
    np.savetxt(f'./Average/Shear-Stress-Mu-{RUPTURE_POSITION}-Y-Averaged.csv', np.column_stack((bin_centers, muu_means)), delimiter=",", header="x,Mu", comments="", fmt="%.6e")
    ...
    if np.count_nonzero(mid_mask_z) > 0:
        ...
        for yy, ss, mu in zip(y_arr[mid_mask_z], s_arr[mid_mask_z], mu_arr[mid_mask_z]):
        ...
        for bi in range(NUM_Y_BINS):
            ...
            mu_midline_y[bi] = mu_arr[in_bin][idx_local]
    ...
    np.savetxt(f'./{MIDLINE_FOLDER}/Shear-Stress-{RUPTURE_POSITION}-MidlineZ-S12-vs-Y.csv', np.column_stack((y_centers_mid, s12_midline_y)), delimiter=",", header="x,S12", comments="", fmt="%.6e")
    # 繪圖：Y vs MU（取 Z = Z-mid 的中線帶）
    plt.figure(figsize=(8, 4))
    plt.plot(y_centers_mid, mu_midline_y, 'o-')
    plt.xlabel('Y Position')
    plt.ylabel('MU at Z-midline (MPa)')
    plt.title(f'Midline (Z={z_mid:.3f}) Shear Modulus MU vs Y  |  tol≈{tol_ratio_z*100:.1f}% span')
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'./{MIDLINE_FOLDER}/Shear-Stress-{RUPTURE_POSITION}-MidlineZ-MU-vs-Y.pdf', dpi=300)
    np.savetxt(f'./{MIDLINE_FOLDER}/Shear-Stress-{RUPTURE_POSITION}-MidlineZ-MU-vs-Y.csv', np.column_stack((y_centers_mid, mu_midline_y)), delimiter=",", header="x,MU", comments="", fmt="%.6e")
