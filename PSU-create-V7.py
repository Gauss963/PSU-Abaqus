...
# ---------------------------------------------------------------------------
# 2-2  Center block – chamfer added later
# ---------------------------------------------------------------------------
...
def add_chamfer(part, size):
    """Chamfer every vertical edge by (size, size) using Abaqus 2024 API."""
    # collect edges parallel to global Y
    verts = part.vertices
    vertical_edges = []
    for e in part.edges:
        v1, v2 = e.getVertices()
    ...
    # Abaqus 2023+ prefers ChamferEdge; pass a *tuple* of Edge objects
    part.ChamferEdge(edgeList=tuple(vertical_edges), lengths=(size, size))
...
asm.rotate(instanceList=(inst_right,), axisPoint=(0,0,0),
...
face_norm = inst_left.faces.getByBoundingBox(xMin=x_norm-1e-3, xMax=x_norm+1e-3)
